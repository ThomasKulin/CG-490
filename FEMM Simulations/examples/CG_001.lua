...
Results_file =          "Run1_CG_res.csv"
Results_overview_file = "Run1_CG_ovvw.csv"
...
end_time = 10E-3 -- end point at 10 ms

m = 0.018 -- mass of slug; This relates to a steel slug, 9.6mm dia, 50mm length, rounded tip. It weighs 25g.

V0 = 52 -- [Volt]; start voltage in Cap

Cap = 6800.0E-3 -- capacitor in F
...
z0 = 0.0 -- mm into coil; negative numbers mean the slug tip is outside of the coil
print("z0 chosen:",z0)

v = 0 -- [m/s] start speed of slug

total_length = 70 -- mm; length of coil

layers = 3 -- number of wire layers
...
Number = 1 -- Number of simulation

-- move slug to starting point in order to determine starting inductance
      mi_clearselected()
      mi_seteditmode("group")

    if z0~=-5 then
      mi_selectgroup(1) -- slug
      mi_movetranslate(0,z0 + 5.0) -- mm up
     end
      mi_refreshview()
...
-- Initially, inner edge is at r =  6mm
--            outer edge is at r =  8mm

-- r-direction; the size depends on wire diameter and number of layers
--    starting size 6,-25, 8,25, coil has 2mm thickness
      mi_clearselected()
      mi_selectrectangle(7,-30,10,30) -- select outer points
      mi_movetranslate(-2 + layers * diameter[awg],0,0)
      mi_clearselected()


-- set length of coil, z-direction
--    starting size   6,-25,  r of width,25; coil has 50mm length

      if (total_length ~= 50.0) then
        mi_selectrectangle(5,20,89,30) -- select upper points
        mi_movetranslate(0, 50 - total_length,0)
      end
...
-- set number of turns for electrical
      ...
      mi_setblockprop("18 AWG",0,0.3,"Coil",0,2,turns)
      mi_refreshview()
