...
def main():
    ...
    z = [[pos_start/100 for p in range(len(time_iter))] for r in range(len(var_iter))]  # z position
    ...
    for r in range(len(var_iter)):
        ...
        coil.setCoilCurrent(coilCurrent)
        ...
        print("Variable Iteration:", r, "\tLayers:", numLayers, "\t Turns:", numTurns, '\tCoil Length:', round(projectile.getLength(), 6), '[cm]', '\tCurrent:', round(coilCurrent, 6), '[A]\tIR:', round(projectile.getRadius(), 6), '[cm]', '\tOR:', round(outerRad, 6), '[cm]', '\tWire Length:', wireLength, '[m]', '\tWire Resistance:' + str(round(R,4)) + '[ohm]', '\tInductance', L, '[uH]')
        ...
        for i in range(1, len(time_iter)):# start at 1 because initial conditions are zero and I want that to be reflected in data
            ...
            print("Iteration # " + str(i) + '\tTime = ' + str(round(time_iter[i], 6)) + '\tz = ' + str(
                round(z[r][i], 6)) + '\tv = ' + str(round(v[r][i], 6)) + '\ta = ' + str(round(a[r][i], 6)))

            if(z[r][i] > pos_end/100):
                break
