...
# coil params
coil_length = 4  # coil length [cm]
piston_length = 7.5  # piston length [cm]
r_inner = 0.32  # coil inner radius [cm]
wallThickness = 0.16
wireDiameter = 0.51  # coil wire diameter [mm]
wireResistivity = 0.0842  # coil wire resistance per length [ohm/m]
# circuit params
circuitResistance = 0.15  # [ohm]
# projectile params
piston_mass = 0.022  # [kg]
pos_start = 2  # [cm]
pos_end = 6
v_start = 0  # starting velocity in [m/s]
k_spring = 600  # spring constant [N/m]
# time [s]
time_start = 0
time_stop = 2e-2
time_step = 5e-4
...
def main():
    ...
    coil.triggerPosition = pos_start
    ...
    dataDir = '..\\'+'Data\\injector\\TimeRLSweep ' + DateTime
    ...
    for r in range(len(var_iter)):
        ...
        projectile.setDimensions(r_inner, piston_length)

        if testType == 'resistance':
            outerRad, exactLayers, numLayers, R, wireLength, numTurns, L = coil.fixedResistance(r_inner + wallThickness, coil_length, var_iter[r], wireDiameter / 10, wireResistivity)
            worksheet = initializeSheet(workbook, 'R' + str(var_iter[r]))
        else:
            outerRad, exactLayers, numLayers, wireLength, R, numTurns, L = coil.fixedInductance(r_inner + wallThickness, coil_length, var_iter[r], wireDiameter / 10, wireResistivity)
            worksheet = initializeSheet(workbook, 'L' + str(var_iter[r]))

        coil.setDimensions(r_inner + wallThickness, outerRad, coil_length)
        ...
        for i in range(1, len(time_iter)):# start at 1 because initial conditions are zero and I want that to be reflected in data
            ...
            force = femm.mo_blockintegral(19)  # calculate force applied to projectile
            f[r][i] = force - k_spring * (z[r][i-1] - z[r][0])  # calculate the net force

            # calculate kinematics stuffs
            a[r][i] = f[r][i] / piston_mass
            v[r][i] = v[r][i - 1] + a[r][i] * time_step
            z[r][i] = z[r][i - 1] + v[r][i] * time_step

            worksheet.write(i, 0, round(time_iter[i], 6))
            worksheet.write(i, 1, round(z[r][i], 6))
            worksheet.write(i, 2, round(v[r][i], 6))
            worksheet.write(i, 3, round(a[r][i], 6))
            worksheet.write(i, 4, round(f[r][i], 6))
            worksheet.write(i, 5, round(I[r][i], 6))
            print("Iteration # " + str(i) + '\tTime = ' + str(round(time_iter[i], 6)) + '\tz = ' + str(
                round(z[r][i], 6)) + '\tv = ' + str(round(v[r][i], 6)) + '\ta = ' + str(round(a[r][i], 6)) + '\tI = ' + str(round(I[r][i], 6))
                  + '\tSpring Force = ' + str(round(force - f[r][i], 6)))
...
def initializeSheet(workbook, name):
    ...
    worksheet.write(0, 1, 'Position [m]')
    worksheet.write(0, 2, 'Velocity [m/s]')
    worksheet.write(0, 3, 'Acceleration [m/s^2]')
    worksheet.write(0, 4, 'Force [N]')
    worksheet.write(0, 5, 'Current [A]')
